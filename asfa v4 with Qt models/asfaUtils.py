...
class Transfer(QRunnable):
    ...
    def __init__(self, src, dst, size, model=None, indx=None, task="copy"):
        super().__init__()
...
def get_files(folder: str) -> Generator:
    ...
    for entry in os.scandir(folder):
        ...
        if entry.is_file() and not (entry.name.startswith(common.other_file_excludes) or entry.name in common.file_excludes):
...
def get_file_details(filename: str):
    ...
    """
    name, folder = os.path.basename(filename), os.path.dirname(filename)
    ext = os.path.splitext(name)[-1]
    ext = f"{ext.strip('.').upper()} File"
    size = common.convert_bytes(os.path.getsize(filename))
