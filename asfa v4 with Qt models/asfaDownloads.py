...
class Worker(QRunnable):
    ...
    __slots__ = (
        ...
        "file_size", "dst_dir", "model_index", "settings", "cwdir"
    )

    def __init__(self, file: str, cwd: str, size: int, indx=None):
        super().__init__()
    ...
    def run(self):
        ...
        try:
            ...
            self.ftp.connect("192.168.43.211", 2121)
            self.ftp.login()
        ...
        # errror when the file is not found
            ...
            print("[FileNotFound]:", self.filename, e)
            self.ftp.quit()
            self.downloading_file.close()
            os.unlink(self.job_id)
        ...
        # write permission denied (PermissionError)
            ...
            self.signals.finished.emit(self.job_id, self.model_index)
...
class WorkerManager(QAbstractListModel):
    ...
    def __init__(self):
        ...
        self.max_threads = self.downloads_threadpool.maxThreadCount()
        print("Can download only %d file(s) at a time" % self.max_threads)
    ...
    def notify_status(self):
        ...
        self.status.emit(f"{len(self._workers)} remaining")
