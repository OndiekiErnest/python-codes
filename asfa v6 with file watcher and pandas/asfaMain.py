...
class WorkerManager(TransferWindow):
    ...
    def remove_folder(self):
        if self.folder_to_remove:
            shutil.rmtree(self.folder_to_remove, ignore_errors=True)
            self.folder_to_remove = ""
...
class DiskManager(QWidget):
    ...
    def create_files_list(self, generators: Iterable[Generator], root_folder):
        ...
        model = DiskFilesModel(generators, ("Name", "File Path", "Type", "Size"), root_folder)
...
class Controller(MainWindow):
    ...
    def __init__(self):
        ...
        # refresh btn
        self.disk_man.refresh_files_btn.clicked.connect(self.refresh_disk_files)
    ...
    def share_context_menu(self, e):
        ...
        if self.isShareTableFocused:
            ...
            if selected_items:
                ...
                from_row = self.share_from_row
                # if any of the selected items not downloaded
                any_not_downloaded = any([(from_row(x)[1] == 0 and from_row(x)[2] == "File") for x in selected_items])
                # if all downloaded
                all_downloaded = all([(from_row(x)[1] and from_row(x)[2] == "File") for x in selected_items])
                # if folder
                is_folder = all([from_row(x)[2] == "Folder" for x in selected_items])

                if any_not_downloaded:
                    context.addAction(self.download_icon, "Download", self.download_clicked)
                    context.addSeparator()

                if all_downloaded or is_folder:
                    context.addAction(self.open_icon, "Open")
                    context.addSeparator()

                if all_downloaded:
                    context.addAction(self.delete_icon, "Delete")
        ...
        try:
            ...
            if all((src, dst)):
                if copy:
                    ...
                    self.worker_manager.transfer_to.setText(f"Moving to '{dst}'")
                    self.worker_manager.folder_to_remove = src
