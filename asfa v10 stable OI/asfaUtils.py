...
class NetMonitor(QThread):
    ...
    def kill(self):
        """ kill the while loop """
        utils_logger.info("Stopping IP address monitor")
        self.alive = 0
...
class DeviceListener(QThread):
    ...
    def list_drives(self) -> list:
        ...
        try:
            return [i for i in GetLogicalDriveStrings().split("\x00") if isRemovable(i)]
    ...
    def close(self):
        utils_logger.debug("Stopping USB drive listener")
        self.is_running = 0
